...
function Fixture() {
  ...
  return <div className="flex"><Sidebar items={items} activeSection={active} onSectionChange={setActive} canCustomize customizeMode={customize} onToggleCustomize={() => setCustomize(v => !v)} onMoveItem={(id, direction) => setItems(current => { const next = [...current]; const index = next.findIndex(item => item.id === id); const target = index + (direction === "up" ? -1 : 1); if (target >= 0 && target < next.length) [next[index], next[target]] = [next[target], next[index]]; return next; })} /><main className="min-w-0 flex-1 p-5"><p>Current section: {active}</p><SetupImportHub supabase={db} organizationId="fixture-org" userId="fixture-user" actorProfileId="fixture-profile" createAuditLog={async () => {}} onNavigate={setActive} onImported={() => {}} /></main><HelpCenter userId="setup-import-fixture" /></div>;
}
