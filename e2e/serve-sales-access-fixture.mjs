...
const fixture = ["entry-navigation", "meeting-followup", "tutorial-barcode"].includes(process.env.TRADEOS_FIXTURE_NAME) ? process.env.TRADEOS_FIXTURE_NAME : "sales-access";
...
const css = fixture === "tutorial-barcode" ? readdirSync(stylesDirectory).filter(name => name.endsWith(".css")).map(name => readFileSync(new URL(name, stylesDirectory), "utf8")).join("\n").replace(/@font-face\{[^}]*\}/g, "") : "";
