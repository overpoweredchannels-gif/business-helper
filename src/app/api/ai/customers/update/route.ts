...
export async function POST(req: NextRequest) {
  try {
    ...
    const updateRes = await supabase.from("customers").update(update).eq("id", customer_id).eq("organization_id", organizationId);

    if (updateRes.error) {
      return NextResponse.json(
        { ok: false, error: `Failed to update customer: ${updateRes.error.message}` },
        { status: 500 }
      );
    }
