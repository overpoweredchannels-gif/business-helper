...
export const customersImportConfig: EntityImportConfig = {
  ...
  tableName: "customers",
  existingColumns: "id, customer_name, shop_name, phone, whatsapp, city, area, customer_type, credit_policy, credit_limit, credit_days, allow_over_limit, allow_overdue_sales, preferred_payment_method, is_active, notes, assigned_salesman_id, assigned_territory_id",
  ...
  export: {
    ...
    async fetchData(orgId, filters) {
      ...
      let query = supabase
        .from("customers")
        .select(`
          id, customer_name, shop_name, phone, whatsapp, city, area,
