...
export const guideTopics: GuideTopic[] = [
  ...
  { title: "Barcode and quick sale", description: "Use a USB/Bluetooth barcode scanner in keyboard mode. It types the barcode into the focused scan field; no special device pairing inside TradeOS is needed for this mode.", steps: ["In Products, click Ready to scan, scan the barcode, fill the remaining details and save. Keep leading zeroes in the barcode.", "Choose Create a sale / Scan products from the dashboard and select a customer. You can create a normal walk-in customer record for counter sales.", "Choose whether each scan adds one main unit or one sub-unit. Scan to add the matching saved product. Scanning again increases its quantity.", "Adjust quantity/price as needed. Click Ready to scan after editing another field. Unknown or duplicate barcodes must be corrected.", "Review and save explicitly. Employee sales still require owner approval. Only keep the customer for the next sale when you intentionally want the same customer."] },
