...
export function PreparedFileReview({ headers, rows, mapping, fields, entity }: { headers: string[]; rows: string[][]; mapping: ColumnMapping; fields: ImportFieldDef[]; entity: string }) {
  ...
  return <section className="my-4 rounded-xl border border-primary/30 bg-card p-4" aria-label="Prepared import file">
    ...
    <p className="my-2 text-sm text-muted-foreground">Your mapping arranges the original records into TradeOS columns. Use the AI suggestions above if needed, then review every page below. Downloading does not save records. Required blanks remain blank; optional settings you did not map are omitted.</p>
