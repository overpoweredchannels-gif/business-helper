...
import { setupFields, type SetupEntity } from "@/lib/tradeos/data-quality";
...
export function SetupReview({ entity, onSaved }: { entity: SetupEntity; onSaved: () => void }) {
  ...
  return <section className="mb-4 rounded border border-amber-300 bg-amber-50 p-4 text-slate-900">
    <div className="flex flex-wrap gap-3"><strong>{rows.length} {entity} · {rows.filter(r => r.missing.length).length} with unset settings</strong><button type="button" onClick={() => { setExpanded(!expanded); void load(); }} className="underline">{expanded ? "Close" : "Review and bulk edit"}</button></div>
    ...
    {expanded && <div className="mt-3 space-y-3">
      ...
      <div className="flex flex-wrap gap-3"><button type="button" onClick={() => setSelected(filtered.map(row => row.id))}>Select all {filtered.length} matching records</button><button type="button" onClick={() => setSelected([])}>Clear selection</button><span>{selected.length} selected</span></div>
      ...
      <div className="flex gap-3"><button disabled={page === 0} onClick={() => setPage(page - 1)}>Previous</button><span>Page {page + 1} of {Math.max(1, Math.ceil(filtered.length / 50))}</span><button disabled={(page + 1) * 50 >= filtered.length} onClick={() => setPage(page + 1)}>Next</button></div>
      ...
      <button type="button" disabled={busy || !selected.length || value === ""} onClick={() => setConfirmation({ ids: [...selected], field, value })}>Apply to selected records</button></div>
      ...
      <button type="button" disabled={busy} onClick={async () => {
        ...
        catch (error) { setMessage(error instanceof Error ? error.message : "Update failed"); await load(); onSaved(); } finally { setBusy(false); }
