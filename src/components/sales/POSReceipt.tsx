...
export type Receipt = { scope: string; business: string; number: string; date: string; customer: string; total: number; payment: string; lines: Array<{ name: string; quantity: string; unit: string; price: string; discount: string; bonus?: string }> };
export function POSReceipt({ receipt }: { receipt: Receipt }) {
  ...
  return <>
    ...
    {createPortal(<dialog ref={dialog} data-pos-receipt data-context-help-skip className="m-auto max-h-[90dvh] w-[min(420px,95vw)] overflow-auto rounded-xl border bg-white p-6 text-black shadow-xl backdrop:bg-black/40">
      ...
      <div className="mt-3 flex justify-between border-t pt-3 text-lg font-bold"><span>Total</span><span>{money(receipt.total)}</span></div><p className="text-sm">{receipt.payment === "cash" ? "Paid in cash" : receipt.payment === "unreconciled" ? "Payment needs reconciliation" : "Credit sale"} · Total includes invoice adjustments.</p>
