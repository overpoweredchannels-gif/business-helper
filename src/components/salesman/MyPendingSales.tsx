...
export function MyPendingSales() {
  ...
  useEffect(() => {
    ...
    authorizedFetch("/api/sales/drafts/mine").then(async response => {
      ...
      if (active) setRows((data.drafts ?? []).filter((row: { status: string }) => ["pending_approval", "rejected", "draft"].includes(row.status)));
  ...
  return <section className="mt-4 rounded-xl border border-border p-4"><h2 className="font-semibold">My submitted sales</h2>
    ...
    {loading && <p className="text-sm" role="status">Loading your submissions...</p>}
    {!loading && !error && !rows.length && <p className="text-sm">No pending submissions.</p>}
