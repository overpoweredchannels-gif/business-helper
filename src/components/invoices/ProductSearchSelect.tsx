...
export function ProductSearchSelect({ value, onChange, products, label = "Product" }: {
  value: string;
  ...
  return <div className="flex min-w-0 flex-col gap-1">
    <input type="search" aria-label={`Search ${label.toLowerCase()}`} placeholder="Search name, brand or SKU" value={search}
    ...
    <select required aria-label={label} value={value} onChange={event => onChange(event.target.value)}
      ...
      <option value="">{filtered.length ? "Select product" : "No matching products"}</option>
